...
public:
explicit MLModelServiceTFLite::MLModelServiceTFLite(vsdk::Dependencies dependencies,
                                                    vsdk::ResourceConfig configuration)
    : MLModelService(configuration.name()),
      MLModelServiceTFLite::state_(configure_(std::move(dependencies), std::move(configuration))) {}
...
void MLModelServiceTFLite::MLModelServiceTFLite::stop(const vsdk::ProtoStruct &extra) noexcept
{
    return stop();
}

/// @brief Stops the MLModelServiceTFLite from running.
void MLModelServiceTFLite::stop() noexcept
{
...
struct metadata MLModelServiceTFLite::metadata(const vsdk::ProtoStruct &extra)
{
...
private:
struct MLModelServiceTFLite::state_;
...
static MLModelServiceTFLite::std::unique_ptr<struct state_> configure_(vsdk::Dependencies dependencies,
                                                                       vsdk::ResourceConfig configuration)
{
...
static MLModelServiceTFLite::MLModelService::tensor_info::data_types service_data_type_from_tflite_data_type_(
    TfLiteType type)
{
    switch (type)
    {
...
struct MLModelServiceTFLite::state_ : public tflite::ErrorReporter
{
    explicit MLModelServiceTFLite::state_(vsdk::Dependencies dependencies, vsdk::ResourceConfig configuration)
...
MLModelService::tensor_views MLModelServiceTFLite::make_tensor_view_(const MLModelService::tensor_info &info,
...
template <typename T>
MLModelService::tensor_views MLModelServiceTFLite::make_tensor_view_t_(const MLModelService::tensor_info &info,
                                                                       const TfLiteTensor *const tflite_tensor)
{
